...
# from gui import mostrar_vista_previa  # Eliminado - solo interfaz gráfica
...
class PDFHandler(FileSystemEventHandler):
    def __init__(self, config, observer=None, root=None):
        ...
        self.archivos_extraidos = set()  # Archivos recién extraídos de ZIP
        self.ignorar_archivos = set()  # Archivos a ignorar permanentemente
        self.carpeta_numero = getattr(self, 'carpeta_numero', 1)  # Número de carpeta para logs
    ...
    def _procesar_archivo_seguro(self, ruta, extension):
        try:
            ...
            if not resource_monitor.can_process_file():
                logging.warning(f"[Carpeta {getattr(self, 'carpeta_numero', '?')}] ⚠️ RECURSOS INSUFICIENTES para: {Path(ruta).name}")
                return
            ...
            if extension == '.zip':
                logging.info(f"[Carpeta {getattr(self, 'carpeta_numero', '?')}] 🗃️ Procesando ZIP: {Path(ruta).name}")
                self.extraer_zip(ruta)
            elif extension in ['.txt', '.zpl']:
                logging.info(f"[Carpeta {getattr(self, 'carpeta_numero', '?')}] 📄 Procesando TXT/ZPL: {Path(ruta).name}")
                self.imprimir_txt(ruta)
            elif extension == '.pdf':
                mensaje = f"[Carpeta {getattr(self, 'carpeta_numero', '?')}] 📄 Procesando PDF: {Path(ruta).name}"
    ...
    def _esperar_archivo_completo(self, ruta, max_intentos=20):
        ...
        for intento in range(max_intentos):
            try:
                ...
                tamaño_actual = Path(ruta).stat().st_size
                
                # Si el tamaño no cambió en 2 intentos consecutivos, probablemente esté completo
                    ...
                    if intentos_sin_cambio >= 2:
                        ...
                        try:
                            ...
                            with open(ruta, 'rb') as f:
                                f.read(1)
                            # Archivo legible pero no exclusivo, esperar un poco más
                            intentos_sin_cambio = 1
                else:
                    intentos_sin_cambio = 0
    ...
    def on_created(self, event):
        ...
        if not SecurityValidator.validate_filename(Path(ruta).name):
            logging.error(f"[Carpeta {getattr(self, 'carpeta_numero', '?')}] ⚠️ NOMBRE INSEGURO: {Path(ruta).name}")
        ...
        try:
            ...
            logging.error(f"Error al verificar ruta canónica: {e}")
            return
        ...
        # Ignorar archivos recién extraídos de ZIP
    ...
    def extraer_zip(self, ruta_zip):
        ...
        if not zip_path.exists():
            logging.warning(f"📁 Archivo ZIP no encontrado: {ruta_zip}")
            return False
            
        if not zip_path.is_file():
            logging.warning(f"📁 No es un archivo: {ruta_zip}")
            return False
            
        try:
            size_mb = zip_path.stat().st_size / (1024 * 1024)
            if size_mb > MAX_ZIP_SIZE_MB:
                logging.error(f"📁 Archivo ZIP demasiado grande: {size_mb:.1f}MB")
                return False
        except Exception as e:
            logging.error(f"Error al verificar tamaño del ZIP: {e}")
            return False
            
        carpeta_destino = self.config["entrada"]
        
        if not validar_directorio(carpeta_destino):
            logging.error(f"📁 Directorio no válido: {carpeta_destino}")
            return False
        ...
        try:
            ...
            with zipfile.ZipFile(str(zip_path)) as zip_ref:
                ...
                for info in archivos_seguros:
                    ...
                    logging.info(f"Archivo extraído: {info.filename}")
            ...
            logging.error(f"📁 Error al extraer ZIP: {e}")
            return False
        except Exception as e:
            logging.exception(f"📁 Error crítico al procesar ZIP: {e}")
            return False
    ...
    def imprimir_txt(self, ruta_txt):
        ...
        if not txt_path.exists() or not txt_path.is_file():
            logging.warning(f"📄 Archivo no válido: {ruta_txt}")
            return False
            
        try:
            size_kb = txt_path.stat().st_size / 1024
            if size_kb > MAX_TXT_SIZE_KB:
                logging.error(f"📄 Archivo demasiado grande: {size_kb:.1f}KB")
                return False
        except Exception as e:
            logging.error(f"Error al verificar tamaño: {e}")
            return False
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if self.root:
                        logging.info(f"[Carpeta {self.carpeta_numero}] ✅ MOSTRANDO VISTA PREVIA para: {Path(txt_path).name}")
                        self.root.after(0, lambda: mostrar_vista_previa(response.content, contenido, callback_imprimir))
                    else:
                        logging.warning(f"[Carpeta {self.carpeta_numero}] ⚠️ SIN GUI - IMPRIMIENDO DIRECTAMENTE: {Path(txt_path).name}")
                ...
                logging.warning("📄 Timeout en vista previa")
                return self._procesar_impression(str(txt_path), contenido)
            except Exception as e:
                logging.warning(f"📄 Error en vista previa: {e}")
                return self._procesar_impression(str(txt_path), contenido)
    ...
    def procesar_pdf(self, ruta_pdf):
        """Procesa PDF con o sin recorte según configuración"""
        ...
        # Verificar configuración de recorte ANTES de procesar
        ...
        logging.info(f"Configuración de recorte PDF: {recortar_habilitado} (carpeta: {self.config.get('entrada', 'N/A')})")
        
        if not recortar_habilitado:
            # Si no se debe recortar, imprimir PDF directamente
            logging.info("PDF se enviará sin recortar - imprimiendo directamente")
        ...
        try:
            ...
            if not pdftoppm_exe:
                logging.error("No se encontró pdftoppm.exe para recorte")
                return False
            ...
            cmd = [
                pdftoppm_exe,
                "-png", "-singlefile", "-r", "600",  # Doble resolución
            ...
            if temp_img.exists():
                ...
                logging.error("No se generó la imagen del PDF")
                return False
                
            # Recortar la imagen
            img_recortada = self._recortar_etiqueta(img)
            logging.info("PDF recortado automáticamente")
    ...
    def _recortar_etiqueta(self, img):
        """Recorta automáticamente la etiqueta del PDF"""
        try:
            ...
            if min_x >= width or min_y >= height or max_x <= 0 or max_y <= 0:
                logging.warning("No se detectó contenido para recortar")
                return img
            ...
            # Verificar que realmente se recortó
            if img_recortada.size == img.size:
                logging.error("ERROR: La imagen no se recortó correctamente")
            else:
        ...
        try:
            ...
            try:
                ...
                if api_sent:
                    ...
                    return imprimir_png(png_path, impresora_a_usar)
    ...
    def _send_to_api_png(self, filename, png_content, printer, copies):
        ...
        try:
            ...
            response = requests.post(
                ...
                json=data,
                timeout=10  # Más tiempo para PNGs grandes
            )
    ...
    def _imprimir_pdf_directo(self, ruta_pdf):
        ...
        try:
            ...
            logging.info(f"Preparando PDF para impresión directa: {ruta_pdf}")
            ...
            if pdftoppm_exe:
                ...
                cmd = [
                    pdftoppm_exe,
                    "-png", "-singlefile", "-r", "200",  # Mejor resolución para vista previa
                    "-aa", "yes",
                    str(ruta_pdf),
                    str(temp_img.with_suffix(""))
                ]
    ...
    def _finalizar_pdf_directo(self, ruta_pdf, temp_img, impresora=None):
        """Finaliza la impresión directa del PDF usando métodos compatibles con red"""
        try:
            impresora_a_usar = impresora if impresora else self.config.get("impresora")
            logging.info(f"Intentando imprimir PDF directamente en: {impresora_a_usar}")
            
            # Convertir PDF a imagen y usar el método PNG que ya funciona
            if not temp_img:
                ...
                if pdftoppm_exe:
                    ...
                    logging.error("No se encontró poppler para conversión")
                    return False
    ...
    def _procesar_impression(self, ruta_txt, contenido, impresora=None):
        try:
            ...
            # SIEMPRE enviar a API primero (incluso con impresora inválida)
            ...
            if api_sent:
                ...
                # La API se encargará de manejar impresoras inválidas
                success = True
            else:
                # Fallback a impresión directa solo si API no está disponible
                logging.info("API no disponible, usando impresión directa")
                
                # Validar impresora solo para impresión directa
                if impresora_a_usar == "IMPRESORA_NO_CONFIGURADA":
            ...
            logging.exception(f"🖨️ Error crítico: {e}")
            return False
